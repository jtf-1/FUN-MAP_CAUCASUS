...
DYNDECK.ship = {
    {
        ...
        coalitionID = 2,
        templates = {
            {name = "forrestal_flex4c234", menutext = "Flex 4 spawn cat 234"},
        }
    },
    {
        name = "CVN-72 Lincoln",
        ...
        coalitionID = 2,
        templates = {
            {name = "sc_flex4c234", menutext = "Flex 4 spawn cat 234"},
        }
    },
}

function DYNDECK:cleardeck(shipID, coalitionID)
    local staticPrefix = "dyndeck_" .. shipID
    do
        local statObj = coalition.getStaticObjects(coalitionID) 					--1 Red coalition 2 blue coalition
            for i, static in pairs(statObj) do
                local staticName = static:getName()
                    if string.match(staticName, staticPrefix .. ".*") then
...
function DYNDECK:applyTemplate(templateName, shipID, coalitionID)
    DYNDECK:cleardeck(shipID, coalitionID)
    DYNDECK[templateName](shipID)
end
...
function DYNDECK:addMenu()
    ...
    -- add ships
    for i, menuship in ipairs(DYNDECK.ship) do
        DYNDECK.menu[menuship.id] = MENU_MISSION:New(menuship.name, DYNDECK.menu)
        for j, template in ipairs(menuship.templates) do
            MENU_MISSION_COMMAND:New(template.menutext, DYNDECK.menu[menuship.id], DYNDECK.applyTemplate, self, template.name, menuship.id, menuship.coalitionID)
        end
        MENU_MISSION_COMMAND:New("Clear Deck", DYNDECK.menu[menuship.id], DYNDECK.cleardeck, self, menuship.id, menuship.coalitionID)  
    end
end

DYNDECK:addMenu()

function DYNDECK.sc_flex4c234(shipID)

    local namePrefix = "dyndeck_" .. shipID
...
function DYNDECK.forrestal_flex4c234(shipID)
